...
# Import domain relationship detector
try:
    from domain_relationship_detector import DomainRelationshipDetector, integrate_with_existing_scraping
    HAS_DOMAIN_RELATIONSHIP_DETECTOR = True
except ImportError:
    HAS_DOMAIN_RELATIONSHIP_DETECTOR = False
    print("Warning: Domain Relationship Detector not available")
...
class AsyncDomainDiscoveryService:
    ...
    def _save_task_to_db(self, task_info: TaskInfo):
        ...
        try:
            ...
            logger.error(f"Task result type: {type(task_info.result) if task_info.result else None}")
            if task_info.result:
                try:
                    logger.error(f"Task result keys: {list(task_info.result.keys()) if hasattr(task_info.result, 'keys') else 'Not a dict'}")
                except:
                ...
                with conn.cursor() as cur:
                    ...
                    for row in cur.fetchall():
                        task_data = {
                            ...
                            'metadata': json.loads(row[8]) if isinstance(row[8], str) and row[8] else (row[8] or {}),
                            'logs': row[9],
                            'error': row[10],
                            'result': json.loads(row[11]) if isinstance(row[11], str) and row[11] else (row[11] or {})
                        }
    ...
    def _extract_providers_from_amass_output(self, amass_output_file: str, domain: str) -> list:
        ...
        ORG_PROVIDER_MAPPING = {
            ...
            'proofpoint': {'name': 'proofpoint', 'confidence': 0.8},
            'pphosted': {'name': 'proofpoint', 'confidence': 0.9},
    ...
    def _save_services_to_neo4j(self, result: ServiceAnalysisResult):
        ...
        logger.info(f"Starting _save_services_to_neo4j function")
        
        if not HAS_NEO4J:
            logger.warning("HAS_NEO4J is False, skipping Neo4j save")
            return
        
        target = result.subdomain if result.subdomain else result.domain
        services_count = len(result.services)
        
        logger.info(f"Neo4j save attempt - Target: {target}, Services count: {services_count}")
        logger.info(f"Neo4j driver status - Driver: {type(self.driver) if hasattr(self, 'driver') else 'No driver'}")
        
        if services_count > 0:
            services_list = [f"{s['service']}:{s['port']}" for s in result.services]
            logger.info(f"Services to save: {', '.join(services_list)}")
            for i, service in enumerate(result.services):
                logger.debug(f"Service {i+1}: {service}")
        ...
        try:
            ...
            if services_count > 0:
                services_list = [f"{s['service']}:{s['port']}" for s in result.services]
                logger.info(f"✓ Successfully saved {services_count} services to Neo4j for {target}: {', '.join(services_list)}")
            else:
    ...
    def _create_service_nodes(self, tx, result: ServiceAnalysisResult):
        ...
        logger.info(f"Starting _create_service_nodes transaction")
        
        current_time = datetime.now().isoformat()
        target = result.subdomain if result.subdomain else result.domain
        is_base = self._is_base_domain(target)
        
        logger.info(f"Transaction details - Target: {target}, Is base domain: {is_base}, Services count: {len(result.services)}")
        
        # Create/update Domain or Subdomain node first
        try:
            if is_base:
                logger.info(f"Creating/updating Domain node for {target}")
                domain_query = """
                    ...
                    SET d.id = COALESCE(d.id, $target),
                        ...
                        d.updated_at = $current_time
                    RETURN d.fqdn as fqdn, d.services_count as count
                """
                domain_result = tx.run(domain_query, target=target, services_count=len(result.services), current_time=current_time)
                domain_record = domain_result.single()
                logger.info(f"Domain node result: {domain_record}")
            else:
                logger.info(f"Creating/updating Subdomain node for {target}")
                subdomain_query = """
                    ...
                    SET s.id = COALESCE(s.id, $target),
                        ...
                        s.updated_at = $current_time
                    RETURN s.fqdn as fqdn, s.services_count as count
                """
                subdomain_result = tx.run(subdomain_query, target=target, services_count=len(result.services), current_time=current_time)
                subdomain_record = subdomain_result.single()
                logger.info(f"Subdomain node result: {subdomain_record}")
        except Exception as e:
            logger.error(f"❌ Failed to create/update {('Domain' if is_base else 'Subdomain')} node: {e}")
            raise
        
        # Create Service nodes with shared/reusable approach
        logger.info(f"Processing {len(result.services)} services...")
        ...
        for i, service in enumerate(result.services):
            ...
            logger.info(f"Processing service {i+1}/{len(result.services)}: {service_name}:{port}/{protocol}")
            
            # Generate shared service properties (not domain-specific)
            name = f"{protocol}/{port}"
            service_type = self._get_service_type(port, service_name)
            # Use generic ID based on port/protocol/service (shared across domains)
            shared_service_id = f"service_{protocol}_{port}_{service_name}"
            
            logger.debug(f"Service details - Shared ID: {shared_service_id}, Name: {name}, Type: {service_type}")
            
            try:
                # First, create or update the shared Service node
                service_query = """
                    MERGE (srv:Service {name: $name, type: $service_type})
                    ON CREATE SET 
                        srv.id = $service_id,
                        srv.port = $port,
                    ...
                    RETURN srv.id as service_id, srv.name as service_name, srv.created_at as created_at
                """
                service_result = tx.run(service_query, 
                    service_id=shared_service_id, name=name, service_type=service_type,
                    port=port, protocol=protocol, service_name=service_name, 
                    current_time=current_time)
                service_record = service_result.single()
                
                if service_record:
                    logger.info(f"✓ Service node ready: {service_record['service_name']} (ID: {service_record['service_id']})")
                else:
                    logger.warning(f"No service record returned for {shared_service_id}")
                
                # Second, create the relationship with domain-specific properties
                node_label = "Domain" if is_base else "Subdomain"
                logger.info(f"Creating relationship {node_label} -> Service")
                ...
                relationship_query = f"""
                    ...
                    ON MATCH SET 
                        r.updated_at = $current_time,
                        r.last_detected_at = $detected_at,
                        r.state = $state
                    RETURN type(r) as relationship_type, r.created_at as created_at
                """
                rel_result = tx.run(relationship_query, 
                    target=target, name=name, service_type=service_type,
                    current_time=current_time, detected_at=service['detected_at'], 
                    state=service['state'])
                rel_record = rel_result.single()
                logger.info(f"✓ Relationship created/updated: {rel_record}")
                
            except Exception as e:
                logger.error(f"❌ Failed to process service {shared_service_id}: {e}")
                raise
        
        logger.info(f"✓ Completed creating all {len(result.services)} service nodes")
    ...
    async def run_tls_analysis(self, domain: str, subdomain: Optional[str], task_id: str) -> TLSAnalysisResult:
        """Run basic TLS analysis for domain or subdomain (backward compatible)"""
        target = subdomain if subdomain else domain
        logger.info(f"Starting basic TLS analysis for {target} (task: {task_id})")
    ...
    def _has_tls_ports(self, target: str) -> bool:
        """Check if target has TLS ports (443 or 8443) open"""
        try:
            import socket
            ...
            for port in tls_ports:
                try:
                    with closing(socket.socket(socket.AF_INET, socket.SOCK_STREAM)) as sock:
                        ...
                        if result == 0:
                            logger.info(f"TLS port {port} is open on {target}")
                            return True
                except Exception as e:
                    logger.debug(f"Failed to check port {port} on {target}: {e}")
                    continue
            
            logger.info(f"No TLS ports (443/8443) found open on {target}")
            return False
            
        except Exception as e:
            logger.error(f"Error checking TLS ports for {target}: {e}")
            return False  # Assume no TLS if we can't check

    def _extract_certificate_data(self, key: str, value: Any, cert_data: dict):
        """Extract and organize certificate data from SSL Labs output"""
        parts = key.split('.')
        if len(parts) >= 3:
            cert_idx = parts[1]
            field = '.'.join(parts[2:])
            
            if cert_idx not in cert_data:
                cert_data[cert_idx] = {}
            
            cert_data[cert_idx][field] = value

    def _extract_endpoint_data(self, key: str, value: Any, endpoint_data: dict):
        """Extract and organize endpoint data from SSL Labs output"""
        parts = key.split('.')
        if len(parts) >= 3:
            endpoint_idx = parts[1]
            field = '.'.join(parts[2:])
            
            if endpoint_idx not in endpoint_data:
                endpoint_data[endpoint_idx] = {}
            
            endpoint_data[endpoint_idx][field] = value

    def _extract_http_transaction_data(self, key: str, value: Any, http_data: dict):
        """Extract HTTP transaction data from SSL Labs output"""
        parts = key.split('.')
        if len(parts) >= 3:
            tx_idx = parts[1]
            field = '.'.join(parts[2:])
            
            if tx_idx not in http_data:
                http_data[tx_idx] = {}
            
            http_data[tx_idx][field] = value
    ...
    async def run_detailed_tls_analysis_with_port_filter(self, domain: str, subdomain: Optional[str], task_id: str, force_scan: bool = False) -> Dict[str, Any]:
        """Run comprehensive TLS analysis using SSL Labs with port filtering"""
        target = subdomain if subdomain else domain
        logger.info(f"Starting detailed TLS analysis with port filtering for {target} (task: {task_id})")
        
        # First check if target has TLS ports (443, 8443) open
        if not self._has_tls_ports(target):
            logger.info(f"Skipping detailed TLS analysis for {target} - no TLS ports (443/8443) detected")
            # Return result indicating no TLS
            return {
                "target": target,
                "ssl_labs_data": {"error": "No TLS ports detected"},
                "scan_metadata": {
                    "scan_time": datetime.now().isoformat(),
                    "force_scan": force_scan,
                    "skip_reason": "No TLS ports (443/8443) available"
                }
            }
        ...
        def run_ssllabs_scan():
            return self._run_ssllabs_analysis_sync(target, task_id, force_scan)
        
        try:
            result = await loop.run_in_executor(self.executor, run_ssllabs_scan)
            ...
            logger.error(f"Detailed TLS analysis failed for {target}: {e}")
            if task_id in self.active_tasks:
                self.active_tasks[task_id].status = TaskStatus.FAILED
                self.active_tasks[task_id].error = str(e)
                self.active_tasks[task_id].completed_at = datetime.now()
    ...
    async def run_detailed_tls_analysis(self, domain: str, subdomain: Optional[str], task_id: str, force_scan: bool = False) -> Dict[str, Any]:
        """Run comprehensive TLS analysis using SSL Labs"""
        target = subdomain if subdomain else domain
        logger.info(f"Starting detailed TLS analysis for {target} (task: {task_id})")
    ...
    def _run_ssllabs_analysis_sync(self, target: str, task_id: str, force_scan: bool = False) -> Dict[str, Any]:
        """Run enhanced SSL Labs analysis synchronously"""
        ...
        try:
            ...
            cmd = [
                "ssllabs-scan-v4",
                "--email", "alf@annymo.tech",
                "--json-flat",
                "--verbosity", "debug"  # Use debug for maximum information
            ]
            ...
            if task_id in self.active_tasks:
                self.active_tasks[task_id].progress = 30
                self._update_task_logs(task_id, f"Running enhanced SSL Labs scan: {' '.join(cmd)}")
            
            # Run the command with timeout
            logger.info(f"Executing enhanced SSL Labs scan: {' '.join(cmd)}")
            ...
            if result.returncode != 0:
                error_msg = f"SSL Labs scan failed with code {result.returncode}: {result.stderr}"
                logger.error(error_msg)
                if task_id in self.active_tasks:
                    self._update_task_logs(task_id, error_msg)
                raise Exception(error_msg)
            
            # Parse the enhanced flat JSON output
            ssl_data = self._parse_enhanced_ssllabs_output(result.stdout)
            
            # Update progress
            if task_id in self.active_tasks:
                self.active_tasks[task_id].progress = 85
            
            # Save enhanced TLS info to Neo4j and PostgreSQL
            self._save_enhanced_tls_info(target, ssl_data)
            
            return {
                "target": target,
                "ssl_labs_data": ssl_data,
                "scan_metadata": {
                    "scan_time": datetime.now().isoformat(),
                    "force_scan": force_scan,
                    "command": ' '.join(cmd),
                    "raw_output_size": len(result.stdout)
                }
            }
            
        except subprocess.TimeoutExpired:
            error_msg = f"SSL Labs scan timed out after 10 minutes for {target}"
            logger.error(error_msg)
            if task_id in self.active_tasks:
                self._update_task_logs(task_id, error_msg)
            raise Exception(error_msg)
        except Exception as e:
            logger.error(f"SSL Labs analysis error for {target}: {e}")
            if task_id in self.active_tasks:
                self._update_task_logs(task_id, f"Error: {str(e)}")
            raise

    def _parse_enhanced_ssllabs_output(self, output: str) -> Dict[str, Any]:
        """Parse the enhanced flat JSON output from SSL Labs scan with comprehensive data extraction"""
        ssl_data = {
            'certificates': {},
            'endpoints': {},
            'protocols': {},
        ...
        for line in output.strip().split('\n'):
            if ':' in line and line.startswith('"') and line.endswith('"'):
                try:
                    # Remove outer quotes and split on first colon
                    ...
                    # Categorize data based on key prefix
                    if key.startswith('certs.'):
                        self._extract_certificate_data(key, value, ssl_data['certificates'])
                    elif key.startswith('endpoints.'):
                        self._extract_endpoint_data(key, value, ssl_data['endpoints'])
                    elif 'protocol' in key.lower() or 'tls' in key.lower() or 'ssl' in key.lower():
                        ssl_data['protocols'][key] = value
                    elif any(vuln in key.lower() for vuln in ['heartbleed', 'freak', 'logjam', 'poodle', 'crime', 'breach']):
                        ssl_data['vulnerabilities'][key] = value
                    elif 'cipher' in key.lower():
                        ssl_data['cipher_suites'][key] = value
                    elif key.startswith('httpTransactions.'):
                        self._extract_http_transaction_data(key, value, ssl_data['http_transactions'])
                    elif key.startswith('hstsPolicy.') or key.startswith('hsts'):
                        ssl_data['hsts_policy'][key] = value
                    elif key.startswith('hpkpPolicy.') or key.startswith('hpkp'):
                        ssl_data['hpkp_policy'][key] = value
                    else:
                        ssl_data['raw_fields'][key] = value
    ...
    def _save_enhanced_tls_info(self, target: str, ssl_data: Dict[str, Any]):
        """Save comprehensive TLS information to databases using enhanced parsing"""
        try:
            # Extract organized data from enhanced parsing
            ...
            cipher_suites = ssl_data.get('cipher_suites', {})
            http_transactions = ssl_data.get('http_transactions', {})
            hsts_policy = ssl_data.get('hsts_policy', {})
            hpkp_policy = ssl_data.get('hpkp_policy', {})
            ...
            with self.driver.session() as session:
                ...
                session.run("""
                    ...
                    SET d.ssl_labs_last_scan = $scan_time,
                        d.ssl_labs_criteria_version = $criteria_version,
                        d.ssl_labs_raw_data = $raw_data,
                        d.ssl_labs_protocols = $protocols,
                        d.ssl_labs_vulnerabilities = $vulnerabilities,
                        d.ssl_labs_cipher_suites = $cipher_suites,
                        d.ssl_labs_hsts_policy = $hsts_policy,
                        d.ssl_labs_hpkp_policy = $hpkp_policy
                """, 
                target=target,
                scan_time=datetime.now().isoformat(),
                criteria_version=raw_fields.get('criteriaVersion', 'unknown'),
                raw_data=json.dumps(ssl_data),
                protocols=json.dumps(protocols),
                vulnerabilities=json.dumps(vulnerabilities),
                cipher_suites=json.dumps(cipher_suites),
                hsts_policy=json.dumps(hsts_policy),
                hpkp_policy=json.dumps(hpkp_policy)
                )
                
                # Save comprehensive certificate information
                for cert_idx, cert in certificates.items():
                    if cert:
                        session.run("""
                            MERGE (d:Domain {name: $target})
                            MERGE (c:SSLCertificate {
                                sha256: $sha256,
                            ...
                            SET c.subject = $subject,
                                ...
                                c.key_algorithm = $key_algorithm,
                                c.key_size = $key_size,
                                c.signature_algorithm = $signature_algorithm,
                                c.alt_names = $alt_names,
                                c.common_names = $common_names,
                                c.chain_position = $chain_position,
                                c.pin_sha256 = $pin_sha256,
                                c.sha1_hash = $sha1_hash,
                                c.revocation_status = $revocation_status,
                                c.ocsp_revocation_status = $ocsp_revocation_status,
                                c.crl_revocation_status = $crl_revocation_status,
                                c.must_staple = $must_staple,
                                c.sct = $sct,
                                c.raw_certificate = $raw_certificate
                            MERGE (d)-[:HAS_CERTIFICATE]->(c)
                        """, 
                        target=target,
                        sha256=cert.get('sha256Hash', f'cert_{cert_idx}'),
                        serial_number=cert.get('serialNumber', f'unknown_{cert_idx}'),
                        ...
                        alt_names=json.dumps(cert.get('altNames', [])) if cert.get('altNames') else '[]',
                        common_names=json.dumps(cert.get('commonNames', [])) if cert.get('commonNames') else '[]',
                        chain_position=int(cert_idx),
                        pin_sha256=cert.get('pinSha256', ''),
                        sha1_hash=cert.get('sha1Hash', ''),
                        revocation_status=cert.get('revocationStatus'),
                        ocsp_revocation_status=cert.get('ocspRevocationStatus'),
                        crl_revocation_status=cert.get('crlRevocationStatus'),
                        must_staple=cert.get('mustStaple', False),
                        sct=cert.get('sct', False),
                        raw_certificate=cert.get('raw', '')
                        )
                
                # Save comprehensive endpoint information
                for endpoint_idx, endpoint in endpoints.items():
                    if endpoint:
                        ...
                        session.run("""
                            ...
                            SET e.grade = $grade,
                                e.grade_trust_ignored = $grade_trust_ignored,
                                e.has_warnings = $has_warnings,
                                e.is_exceptional = $is_exceptional,
                                e.progress = $progress,
                                e.duration = $duration,
                                e.eta = $eta,
                                ...
                                e.status_message = $status_message,
                                e.forward_secrecy = $forward_secrecy,
                                e.freak = $freak,
                                e.heartbleed = $heartbleed,
                                e.heartbeat = $heartbeat,
                                e.logjam = $logjam,
                                e.poodle_tls = $poodle_tls,
                                e.rc4_with_modern = $rc4_with_modern,
                                e.vulnerable_beast = $vulnerable_beast
                            MERGE (d)-[:HAS_SSL_ENDPOINT]->(e)
                        """, 
                        target=target,
                        ip_address=ip_address,
                        grade=grade,
                        ...
                        duration=endpoint.get('duration', 0),
                        eta=endpoint.get('eta', 0),
                        delegation=endpoint.get('delegation', 0),
                        details=json.dumps({k: v for k, v in endpoint.items() if k.startswith('details.')}),
                        server_name=endpoint.get('serverName', ''),
                        status_message=endpoint.get('statusMessage', ''),
                        forward_secrecy=endpoint.get('details.forwardSecrecy'),
                        freak=endpoint.get('details.freak'),
                        heartbleed=endpoint.get('details.heartbleed'),
                        heartbeat=endpoint.get('details.heartbeat'),
                        logjam=endpoint.get('details.logjam'),
                        poodle_tls=endpoint.get('details.poodleTls'),
                        rc4_with_modern=endpoint.get('details.rc4WithModern'),
                        vulnerable_beast=endpoint.get('details.vulnBeast')
                        )
                
                # Save HTTP transaction information
                for tx_idx, tx in http_transactions.items():
                    if tx:
                        session.run("""
                            MERGE (d:Domain {name: $target})
                            CREATE (h:HTTPTransaction {
                                domain: $target,
                                ...
                                fragile_server: $fragile_server
                            })
                            MERGE (d)-[:HAS_HTTP_TRANSACTION]->(h)
                        """,
                        target=target,
                        tx_index=int(tx_idx),
                        request_url=tx.get('requestUrl', ''),
                        request_line=tx.get('requestLine', ''),
                        response_line=tx.get('responseLine', ''),
                        status_code=tx.get('statusCode'),
                        response_headers=json.dumps([{"name": tx.get(f'responseHeaders.{i}.name'), "value": tx.get(f'responseHeaders.{i}.value')} for i in range(20) if tx.get(f'responseHeaders.{i}.name')]),
                        request_headers=json.dumps([tx.get(f'requestHeaders.{i}') for i in range(10) if tx.get(f'requestHeaders.{i}')]),
                        fragile_server=tx.get('fragileServer', False)
                        )
            
            logger.info(f"Saved comprehensive TLS information for {target} to Neo4j: {len(certificates)} certificates, {len(endpoints)} endpoints, {len(http_transactions)} HTTP transactions")
            
        except Exception as e:
            logger.error(f"Failed to save detailed TLS info for {target}: {e}")

    def get_detailed_tls_from_neo4j(self, domain: str) -> Optional[Dict[str, Any]]:
        """Get detailed TLS information from Neo4j"""
        if not HAS_NEO4J:
            return None
        ...
        try:
            with self.driver.session() as session:
                ...
                result = session.run("""
                    ...
                    OPTIONAL MATCH (d)-[:HAS_CERTIFICATE]->(c:SSLCertificate)
                    OPTIONAL MATCH (d)-[:HAS_SSL_ENDPOINT]->(e:SSLEndpoint)
                    RETURN d.ssl_labs_last_scan as last_scan,
                           d.ssl_labs_criteria_version as criteria_version,
                           d.ssl_labs_raw_data as raw_data,
                           COLLECT(DISTINCT {
                               sha256: c.sha256,
                           ...
                           COLLECT(DISTINCT {
                               ...
                               grade: e.grade,
                               has_warnings: e.has_warnings,
                               is_exceptional: e.is_exceptional,
                               progress: e.progress,
                               duration: e.duration,
                               eta: e.eta,
                ...
                if not record or not record['last_scan']:
                    return None
                
                # Parse raw SSL Labs data if available
                raw_data = {}
                if record['raw_data']:
                    try:
                ...
                # Filter out empty certificates and endpoints
                certificates = [c for c in record['certificates'] if c['sha256']]
                endpoints = [e for e in record['endpoints'] if e['ip_address'] != 'unknown']
                ...
                return {
                    ...
                    "endpoints": endpoints,
                    "ssl_labs_raw_data": raw_data,
                    "summary": {
                        "total_certificates": len(certificates),
                        "total_endpoints": len(endpoints),
                        "overall_grade": endpoints[0]['grade'] if endpoints else 'Unknown',
                        "has_warnings": any(e.get('has_warnings', False) for e in endpoints),
                        "is_exceptional": any(e.get('is_exceptional', False) for e in endpoints)
                    }
                }
                
        except Exception as e:
            logger.error(f"Failed to get detailed TLS info from Neo4j for {domain}: {e}")
            return None
    ...
    def _run_tech_analysis_sync(self, domain: str, subdomain: Optional[str], task_id: str) -> TechAnalysisResult:
        ...
        try:
            ...
            # Don't create error technology nodes - just log the error
    ...
    def _extract_provider_from_organization(self, organization: str):
        ...
        provider_mappings = {
            ...
            'proofpoint': {'name': 'ProofPoint', 'type': 'email_security'},
            'pphosted': {'name': 'ProofPoint', 'type': 'email_security'},
    ...
    async def run_web_scraping_analysis(self, domain: str, subdomain: Optional[str], task_id: str) -> dict:
        ...
        try:
            ...
            # 5. NEW: Detect domain relationships from scraped content
            domain_relationships_result = {}
            domain_references = []
            if HAS_DOMAIN_RELATIONSHIP_DETECTOR:
                try:
                    ...
                    logger.warning(f"Domain relationship detection failed for {target}: {e}")
                    domain_relationships_result = {"error": str(e)}
            ...
            result = {
                ...
                "domain_relationships": domain_relationships_result,  # NEW: Domain relationship data
    ...
    async def get_domain_relationships(self, domain: str) -> Dict[str, any]:
        """Get domain relationships for a specific domain"""
        if not HAS_NEO4J:
        ...
        try:
            with self.driver.session() as session:
                ...
                RETURN target.fqdn as target_domain,
                       r.reference_type as reference_type,
                       r.reference_context as reference_context,
                       r.confidence as confidence,
                       r.reference_count as reference_count,
                       r.last_seen as last_seen
                ORDER BY r.confidence DESC, r.reference_count DESC
                """
                
                outgoing_results = session.run(outgoing_query, domain=domain)
                outgoing_references = []
                ...
                for record in outgoing_results:
                    outgoing_references.append({
                        ...
                        "reference_count": record["reference_count"] or 1,
                        "last_seen": record["last_seen"]
                    })
                
                # Get incoming references (domains that reference this domain)
                incoming_query = """
                MATCH (source:Domain)-[r:REFERENCES]->(d:Domain {fqdn: $domain})
                RETURN source.fqdn as source_domain,
                       r.reference_type as reference_type,
                       r.reference_context as reference_context,
                       r.confidence as confidence,
                       r.reference_count as reference_count,
                ...
                incoming_results = session.run(incoming_query, domain=domain)
                incoming_references = []
                ...
                return {
                    ...
                    "total_outgoing": len(outgoing_references),
                    "total_incoming": len(incoming_references)
                }
                
        except Exception as e:
            logger.error(f"Error getting domain relationships for {domain}: {e}")
            return {"error": str(e)}
...
async def get_task_logs(task_id: str):
    ...
    if task.result:
        logs.append("\n--- FULL RESULT DATA ---")
        try:
            logs.append(json.dumps(task.result, indent=2, default=str))
        except Exception as e:
            logs.append(f"Result data (non-serializable): {type(task.result)} - {str(e)}")
            logs.append(f"Result keys: {list(task.result.keys()) if hasattr(task.result, 'keys') else 'Not a dict'}")
...
async def start_tls_analysis(
    domain: str = Path(..., description="Domain to analyze"),
    subdomain: Optional[str] = Query(None, description="Specific subdomain to analyze"),
    force_scan: Optional[bool] = Query(False, description="Force live scan instead of using cache")
):
    """Start enhanced TLS analysis for domain or subdomain with SSL Labs integration"""
    if not discovery_service:
        raise HTTPException(status_code=503, detail="Service not available")
    ...
    # Start async analysis using enhanced TLS model with port filtering
    asyncio.create_task(discovery_service.run_detailed_tls_analysis_with_port_filter(domain, subdomain, task_id, force_scan))
    
    return {"task_id": task_id, "domain": domain, "subdomain": subdomain, "status": "started"}

@app.post("/api/v1/discover/tls-detailed/{domain}", tags=["Discovery"])
async def start_detailed_tls_analysis(
    domain: str = Path(..., description="Domain to analyze"),
    subdomain: Optional[str] = Query(None, description="Specific subdomain to analyze"),
    force_scan: Optional[bool] = Query(False, description="Force live scan instead of using cache")
):
    """Start comprehensive TLS analysis using SSL Labs for domain or subdomain"""
    if not discovery_service:
        raise HTTPException(status_code=503, detail="Service not available")
    ...
    # Check if target has HTTPS (443 or 8443)
    has_https = False
    ...
    if not has_https:
        raise HTTPException(
            status_code=400, 
            detail=f"Target {target} does not have HTTPS service running on ports 443 or 8443"
        )
    ...
    # Start async detailed analysis with port filtering
    asyncio.create_task(discovery_service.run_detailed_tls_analysis_with_port_filter(domain, subdomain, task_id, force_scan))
    
    return {"task_id": task_id, "domain": domain, "subdomain": subdomain, "target": target, "status": "started"}

@app.get("/api/v1/tls-detailed/{domain}", tags=["TLS Analysis"])
async def get_detailed_tls_info(
    domain: str = Path(..., description="Domain to get TLS info for")
):
    """Get comprehensive TLS information from SSL Labs analysis"""
    if not discovery_service:
        raise HTTPException(status_code=503, detail="Service not available")
    
    try:
        tls_info = discovery_service.get_detailed_tls_from_neo4j(domain)
        if not tls_info:
            raise HTTPException(status_code=404, detail=f"No detailed TLS information found for {domain}")
        
        return tls_info
        
    except Exception as e:
        logger.error(f"Failed to get detailed TLS info for {domain}: {e}")
...
async def _run_combined_discovery(domain: str, subdomain: Optional[str], task_id: str, 
    ...
    try:
        ...
        if include_tech:
            try:
                logger.info(f"DEBUG: Running technology analysis for {target}")
                tech_result = await discovery_service.run_tech_analysis(domain, subdomain, f"{task_id}-tech")
                results["tech"] = tech_result
                logger.info(f"DEBUG: Technology analysis completed for {target}")
                if task_id in discovery_service.active_tasks:
                    task = discovery_service.active_tasks[task_id]
                    task.progress = 95
                    logger.info(f"DEBUG: Set progress to 95% for task {task_id}")
                    discovery_service._update_task_logs(task_id, f"Completed technology analysis for {target}")
                else:
                    logger.warning(f"DEBUG: Task {task_id} not found in active_tasks after tech analysis")
            except Exception as e:
                logger.error(f"Technology analysis failed for {target}: {e}")
                results["tech"] = {"error": str(e)}
        
        # Mark task as completed
        logger.info(f"DEBUG: Attempting to mark task {task_id} as completed for {target}")
        if task_id in discovery_service.active_tasks:
            task = discovery_service.active_tasks[task_id]
            logger.info(f"DEBUG: Found task {task_id} in active_tasks, current progress: {task.progress}, status: {task.status}")
            task.status = TaskStatus.COMPLETED
            task.progress = 100
            task.result = results
            task.completed_at = datetime.now()
            logger.info(f"DEBUG: Successfully set task {task_id} to COMPLETED with 100% progress")
            discovery_service._update_task_logs(task_id, f"Completed combined discovery for {target}")
        else:
            logger.error(f"DEBUG: Task {task_id} NOT FOUND in active_tasks when trying to complete it for {target}")
            
        logger.info(f"DEBUG: Combined discovery completed for {target}")
        
    except Exception as e:
        logger.error(f"DEBUG: Combined discovery failed for {target}: {e}")
        ...
        if task_id in discovery_service.active_tasks:
            ...
            task.error = str(e)
            task.completed_at = datetime.now()
            logger.error(f"DEBUG: Marked task {task_id} as FAILED")
        else:
            logger.error(f"DEBUG: Task {task_id} not found in active_tasks when trying to mark as failed")
